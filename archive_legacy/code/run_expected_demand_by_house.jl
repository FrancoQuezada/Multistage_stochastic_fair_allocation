...
function expected_demand_by_house(inst::InstanceM)
    probs = _scenario_probabilities(inst)
    expected = zeros(inst.J)
    for (s_idx, scenario) in enumerate(inst.tree.scenarios)
        for j in 1:inst.J
            expected[j] += probs[s_idx] * sum(inst.d[j, n] for n in scenario)
        end
    end
    return expected
end

function run_expected_demand_report(;
    ...
    avg_d::Float64=100.0,
    dev_d::Float64=10.0,
    out_csv::String="expected_demand_by_house_S6_C4_P4.csv"
)
...
function run_expected_demand_config_set(;
    inst_folder::String="inst/inst2020",
    instance_from::Int=1,
    instance_to::Int=100,
    NBstage::Int=6,
        ...
        for theta in theta_set
            for avg_d in avg_d_set
                for dev_d in dev_d_set
                    println(
                        "Expected demand config -> S=", NBstage,
                        ", C=", childs,
                        ", P=", periods,
                        ", J=", J,
                        ", theta=", theta,
                    ...
                    df_cfg = run_expected_demand_report(
...
function main()
    ...
    out_csv = get(ENV, "OUT_CSV", "expected_demand_by_house_S6_C4_P4.csv")

    df = run_expected_demand_config_set(
        inst_folder=inst_folder,
        instance_from=instance_from,
        instance_to=instance_to,
        NBstage=NBstage,
    ...
    println("Reporte expected demand: ", out_csv)
end
