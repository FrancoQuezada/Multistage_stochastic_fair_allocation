...
| 12 | IN PROGRESS | Calibration and computational design probes |
| 13 | IN PROGRESS | Safe campaign parallelization and deterministic merge |
| 14 | NOT STARTED | Pilot, full campaign, and statistical analysis |
...
| 2026-08-09 | **RESOLVED, negatively**: this problem shows no value of stochastic look-ahead. | The paired pilot (8 cells = 2 battery x 2 uncertainty x `h in {1,4}`, `R = 8`, `[4, 4]`, `NONE`) gives 16 comparisons against `DETERMINISTIC_RH`. Seven are significant and **every one favours the deterministic controller**, by +0.17 % to +0.41 % of operating cost; none favours a stochastic controller. The controllers are not degenerate — 11-18 % of implemented actions differ, and the share of differing battery decisions rises with uncertainty — so the information changes decisions without improving cost. The earlier "bit-identical at low uncertainty" reading was an artefact of `[2, 2]` at `R = 1` comparing sums, and is withdrawn. Mechanism: within the first information stage the deterministic path carries the full process noise whatever the leaf count, while a 16-leaf hedge is partly fitted to its own sample, against a baseline that re-optimizes every `h` periods. |
| 2026-08-09 | `R = 120`, sized by interval precision rather than by power against an observed effect. | Powering against the observed effect demands `R = 106 993` in the cell whose effect is -0.0045 %, which is meaningless. The worst measured standard deviation of the paired difference is 0.52 % of cost, so `R = 105` bounds the paired 95 % interval to +/-0.10 % and `R = 419` to +/-0.05 %; every effect observed is at most 0.41 %. `R` stays a parameter and the standard deviation is re-estimated per cell from the campaign itself. |
| 2026-08-09 | `TWO_STAGE_RH` and `MULTISTAGE_RH` coincide exactly in four of the eight pilot cells. | Not a construction defect: the two views of the common support have materially different geometry (36 vs 47 nodes at 2 leaves, 168 vs 369 at 16). It is the statement that intermediate-stage nonanticipativity does not bind on the implemented block there, since the implemented action lives in the known prefix and the two views differ only beyond it. |
| 2026-08-09 | `PEA` correctly refuses a look-ahead window whose total community demand is zero, which makes short `L` inadmissible on this instance. | The instance has 8 active demand periods out of 24, so a homogeneous-regime window over inactive periods has zero community demand and the proportional rule has no denominator. Observed at `H = L = 4`: `PEA` aborted in all four HOMOGENEOUS cells and completed in all four HETEROGENEOUS ones, which would silently unbalance the design. It does not arise at `L = 24`, where every window spans active periods. Recorded as an admissibility condition on `(L, instance)`, not as a defect: the refusal is correct and the campaign's `L = 24` satisfies it. |
| 2026-08-09 | The merge refuses a shard root holding tasks outside the expected set. | Found by the Stage-13 test set: an unexpected committed shard was silently ignored. That hides two real hazards — leftovers from a previous campaign in the same root, and a merge requested with a smaller `R` than the shards were produced with — either of which publishes a dataset smaller than the evidence on disk. `ShardInventory` gained an `unexpected` category and it blocks the merge like any other defect. |
