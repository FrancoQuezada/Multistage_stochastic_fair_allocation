...
function solve!(inst, solver::String)
    ...
    @variable(model, battery_mode[1:inst.T], Bin)
    ################Constraints###################
    ...
    @constraint(model, battery_discharge_mode[t in 1:inst.T],
        sum(y[j,t] for j in 1:inst.J)<=inst.f_bar*(1-battery_mode[t]))
    @constraint(model, battery_charge_mode[t in 1:inst.T],
        sum(z[j,t] for j in 1:inst.J)<=inst.f_under*battery_mode[t])
