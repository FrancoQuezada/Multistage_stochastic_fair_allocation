...
function _static_validation_snapshot(inst::InstanceM, label::String)
    ...
    return (
        ...
        sizes=(
            s=size(sol.s), I=size(sol.I), G=size(sol.G), z=size(sol.z),
...
function _static_solution_reproducible(first::SolutionM, second::SolutionM)
    ...
    for field in (:costs, :s, :I, :G, :z, :y, :p)
...
function validate_static_demand_share(;
    ...
    @testset "All existing policies regression" begin
        @test sort(collect(keys(after))) == sort(collect(keys(baseline)))
        for label in STATIC_EXISTING_LABELS
            before = baseline[label]
            current = after[label]
            @test current.status == before.status
            @test current.sizes == before.sizes
            @test abs(current.expected_cost - before.expected_cost) <=
                STATIC_REGRESSION_RTOL * max(1.0, abs(before.expected_cost))
            for field in (:costs, :s, :I, :G, :z, :y, :p)
                @test _static_componentwise_regression_ok(
                    getproperty(current, field), getproperty(before, field),
                )
            end
        end
    end
