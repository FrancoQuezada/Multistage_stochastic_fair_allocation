...
function run_restricted_exact_config_set(;
    inst_folder::String = "inst/inst2020",
    instance_from::Int64 = 1,
    instance_to::Int64 = 1,
    fairness_set::Vector{String} = ["PEA", "SA"],
    ...
    out_csv::String = "restricted_exact_report.csv",
    out_csv_house::String = "restricted_exact_report_by_house.csv",
    out_csv_diag::String = "restricted_exact_diagnostics.csv",
    out_csv_diag_house::String = "restricted_exact_diagnostics_by_house.csv",
    ...
    println("Total configuraciones restringidas exactas a ejecutar: ", length(configs))

    for (cfg_idx, cfg) in enumerate(configs)
        println(
            "Running restricted config ", cfg_idx, "/", length(configs),
        ...
        result = solve_restricted_exact(inst, cfg.fairness)
        sol = result.restricted_sol
        metrics = _scenario_metrics(inst, sol, cfg.fairness)
        probs = metrics.probs
        total_prob = metrics.total_prob
        fairness_gap = cfg.fairness == "PEA" ? result.diagnostics.pea_gap : result.diagnostics.sa_gap
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    fairness_set = parse_string_list(get(ENV, "FAIRNESS_SET", "PEA,SA"))
    tree_set = begin
        raw = parse_string_list(get(ENV, "TREE_SET", "6:4:4"))
        [begin
            a = split(x, ":")
            length(a) == 3 || error("TREE_SET invalido: $x")
            (NBstage = parse(Int, a[1]), childs = parse(Int, a[2]), periods = parse(Int, a[3]))
        end for x in raw]
    end
    ...
    out_csv = get(ENV, "OUT_CSV", "restricted_exact_report.csv")
    out_csv_house = get(ENV, "OUT_CSV_HOUSE", "restricted_exact_report_by_house.csv")
    out_csv_diag = get(ENV, "OUT_CSV_DIAG", "restricted_exact_diagnostics.csv")
    out_csv_diag_house = get(ENV, "OUT_CSV_DIAG_HOUSE", "restricted_exact_diagnostics_by_house.csv")
    append_each_config = lowercase(get(ENV, "APPEND_EACH_CONFIG", "true")) in ("1", "true", "yes", "y")

    run_restricted_exact_config_set(
