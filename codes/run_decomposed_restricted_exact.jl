include("heuristics_decomposed_restricted_exact.jl")
using DataFrames
using CSV
...
function _diagnostic_gap(result, fairness::String, J::Int)
    if fairness == "PEA"
        return result.diagnostics.pea_gap
    elseif fairness == "SA"
        return result.diagnostics.sa_gap
    elseif fairness in ("LEXMMFPEA", "MMFPEA", "EMMFPEA")
        return result.diagnostics.actual_gap
    elseif fairness in ("LEXMMFSA", "MMFSA")
        return hasproperty(result.diagnostics, :sa_gap) ? result.diagnostics.sa_gap : fill(NaN, J)
    end
    return fill(NaN, J)
end

function run_decomposed_restricted_config_set(;
    inst_folder::String = "inst/inst2020",
    instance_from::Int64 = 1,
    instance_to::Int64 = 1,
    fairness_set::Vector{String} = ["PEA", "SA", "LEXMMFPEA", "LEXMMFSA"],
    ...
    out_csv::String = "decomposed_restricted_report.csv",
    out_csv_house::String = "decomposed_restricted_report_by_house.csv",
    out_csv_diag::String = "decomposed_restricted_diagnostics.csv",
    out_csv_diag_house::String = "decomposed_restricted_diagnostics_by_house.csv",
    ...
    println("Total configuraciones descompuestas a ejecutar: ", length(configs))

    for (cfg_idx, cfg) in enumerate(configs)
        println(
            "Running decomposed config ", cfg_idx, "/", length(configs),
        ...
        result = solve_decomposed_restricted(
            inst,
            cfg.fairness;
            max_iter = max_iter,
            step_size = step_size,
        ...
        sol = result.heuristic_sol
        metrics = _scenario_metrics(inst, sol, cfg.fairness)
        probs = metrics.probs
        total_prob = metrics.total_prob
        fairness_gap = _diagnostic_gap(result, cfg.fairness, cfg.J)
        iter_count = haskey(result.diagnostics, :history) ? length(result.diagnostics.history) : 0
        ...
        for j in 1:cfg.J
            push!(df_house, (
                ...
                inst_folder,
                sol.status,
                sum(probs[s] * metrics.scenario_pv_house[j, s] for s in 1:size(metrics.scenario_pv_house, 2)) / total_prob,
                minimum(metrics.scenario_pv_house[j, :]),
                maximum(metrics.scenario_pv_house[j, :]),
                sum(probs[s] * metrics.scenario_costs_house[j, s] for s in 1:size(metrics.scenario_costs_house, 2)) / total_prob,
                minimum(metrics.scenario_costs_house[j, :]),
                maximum(metrics.scenario_costs_house[j, :]),
                sum(probs[s] * metrics.reg[j, s] for s in 1:size(metrics.reg, 2)) / total_prob,
                minimum(metrics.reg[j, :]),
                maximum(metrics.reg[j, :]),
                result.total_run_time,
                result.total_model_solve_time
            ))
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    fairness_set = parse_string_list(get(ENV, "FAIRNESS_SET", "PEA,SA,LEXMMFPEA,LEXMMFSA"))
    ...
    max_iter = parse(Int, get(ENV, "MAX_ITER", "50"))
    step_size = parse(Float64, get(ENV, "STEP_SIZE", "0.25"))
    inertia = parse(Float64, get(ENV, "INERTIA", "0.80"))
    gap_tol = parse(Float64, get(ENV, "GAP_TOL", "1e-4"))
    out_csv = get(ENV, "OUT_CSV", "decomposed_restricted_report.csv")
    out_csv_house = get(ENV, "OUT_CSV_HOUSE", "decomposed_restricted_report_by_house.csv")
    out_csv_diag = get(ENV, "OUT_CSV_DIAG", "decomposed_restricted_diagnostics.csv")
    out_csv_diag_house = get(ENV, "OUT_CSV_DIAG_HOUSE", "decomposed_restricted_diagnostics_by_house.csv")
    append_each_config = lowercase(get(ENV, "APPEND_EACH_CONFIG", "true")) in ("1", "true", "yes", "y")
    ...
    run_decomposed_restricted_config_set(
        ...
        append_each_config = append_each_config
    )
end
